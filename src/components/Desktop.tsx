...
const Desktop: React.FC<DesktopProps> = ({ onOpenWindow }) => {
  ...
  return (
    ...
    >
      {/* Right-side desktop folder icons */}
      <div className="absolute right-4 top-10 flex flex-col gap-1">
        {desktopItems.map((item, i) => (
          <motion.div
            key={item.id}
            className={`desktop-icon ${selected === item.id ? 'ring-2 ring-blue-400/50 bg-white/20' : ''}`}
            initial={{ opacity: 0, x: 24 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: 0.12 * i + 0.2, type: 'spring', stiffness: 300, damping: 24 }}
            onClick={(e) => { e.stopPropagation(); setSelected(item.id); closeContext(); }}
            onDoubleClick={(e) => { e.stopPropagation(); onOpenWindow(item.id); }}
