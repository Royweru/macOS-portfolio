...
- [~] Verify real video playback, reduced motion, and media end states. Playback controls now bind to the selected real media element; Add opens a library dialog; next/previous/eject, seek, volume, mute, and compact transport are wired. Browser-extension QA verified the empty-library message and disabled playback/sample rows. Real playback, end-state behavior with an actual asset, and reduced-motion playback still need supplied media and live verification.
...
Media playback implementation and evidence: `plans/weru97-media-playback-controls-2026-09-23.md` and `achievements/chapter-2-media-playback-controls-2026-09-23.md`. Do not mark the visual parity or playback tasks complete until matched source captures and actual audio/video assets have been exercised in the browser.
