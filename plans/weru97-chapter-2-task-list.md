...
- [x] Store the four personal documents as links to same-origin files under `public/text/`; seed read-only Notepad nodes with empty content plus `contentUrl`, fetch and cache the linked text on open, and keep document bodies out of both manifests. Filesystem tests verify each path exists and each VFS node carries the matching path rather than literal path text; TypeScript, lint, tests, and production build pass. Manual browser fetch/display was not repeated because localhost remains stopped.
